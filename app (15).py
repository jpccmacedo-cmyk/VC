...
# A data será lida somente em J3/K3. Não usa nome da aba para identificar data.
COLUNAS_DATA = [10, 11]  # J e K
LINHA_INICIAL_DATA_PADRAO = 3
LINHA_FINAL_DATA_PADRAO = 3
...
# ============================================================
# DATA: SOMENTE COLUNAS J/K NAS LINHAS CONFIGURADAS
# ============================================================
...
def extrair_data_da_aba_pelo_conteudo(aba, linha_inicial_data, linha_final_data):
    """
    Regra única de identificação da data:
    - Não usa nome da aba.
    - Lê somente as células J3/K3.
    - Não procura em outras linhas ou colunas.
    """
    textos = []

    for linha in range(int(linha_inicial_data), int(linha_final_data) + 1):
        ...
        # Tenta interpretar linha a linha primeiro.
        data_linha = interpretar_data_em_texto(" ".join(textos[-2:]))
        if data_linha:
            return data_linha

    # Tenta interpretar tudo que encontrou em J/K no intervalo.
    return interpretar_data_em_texto(" ".join(textos))


def obter_data_real_da_aba(aba, linha_inicial_data, linha_final_data):
    return extrair_data_da_aba_pelo_conteudo(aba, linha_inicial_data, linha_final_data)
...
def encontrar_aba_exata_por_data(wb_origem, data_selecionada, linha_inicial_data, linha_final_data):
    """
    Busca somente abas cuja data em J/K seja igual à data selecionada.
    Não considera nome de aba.
    """
    candidatas = []
    for nome_aba in wb_origem.sheetnames:
        aba = wb_origem[nome_aba]
        data_real = obter_data_real_da_aba(aba, linha_inicial_data, linha_final_data)
    ...
    if candidatas:
        ...
        return candidatas[0]["nome_aba"], "Aba com data real lida somente em J3/K3"

    return None, "Nenhuma aba com data real em J3/K3 igual à data selecionada encontrada"


def encontrar_aba_por_data_ou_gerencial(wb_origem, data_selecionada, linha_inicial_data, linha_final_data, permitir_fallback_gerencial=False):
    nome_aba, criterio = encontrar_aba_exata_por_data(wb_origem, data_selecionada, linha_inicial_data, linha_final_data)
    if nome_aba:
    ...
    if permitir_fallback_gerencial:
        ...
        if aba_gerencial:
            data_gerencial = obter_data_real_da_aba(wb_origem[aba_gerencial], linha_inicial_data, linha_final_data)
            if data_gerencial == data_selecionada:
                return aba_gerencial, "Aba Gerencial usada por opção do usuário e validada pelas J3/K3"

    return None, "Nenhuma aba com data real em J3/K3 igual à data selecionada encontrada"


def listar_datas_disponiveis_arquivos(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro, linha_inicial_data, linha_final_data):
    """
    Lista datas disponíveis lendo somente J3/K3.
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        wb = None
        try:
            status.info(f"Lendo datas em J3/K3: {idx}/{len(arquivos_salvos)} — {item['nome']}")
...
def extrair_data_nome_consolidado(nome_arquivo):
    """
    Usado apenas para impedir que consolidado antigo com data no nome entre na data errada.
    A escolha de abas continua sendo somente por J/K.
    """
...
def copiar_abas_de_consolidado_existente(wb_origem, wb_final, logs, nome_arquivo, data_selecionada, linha_inicial_data, linha_final_data):
    copiadas = []
    for nome_aba in wb_origem.sheetnames:
        ...
        data_real = obter_data_real_da_aba(aba_origem, linha_inicial_data, linha_final_data)
        if data_real != data_selecionada:
            logs.append(
                f"Consolidado existente: aba '{nome_aba}' ignorada porque J3/K3 indica "
...
def processar_arquivos_salvos(
    ...
    logs.append(f"Validação única: data lida somente nas células J3/K3. Nome da aba não é considerado.")
    ...
    for item in itens_consolidados:
        ...
        try:
            ...
            copiadas = copiar_abas_de_consolidado_existente(wb, wb_final, logs, item["nome"], data_selecionada, linha_inicial_data, linha_final_data)
            ...
            if copiadas:
                arquivos_processados.append({
                    ...
                    "criterio": "Abas reaproveitadas de consolidado existente com J3/K3 validado"
                })
    ...
    for item in itens_origem:
        ...
        try:
            ...
            wb = load_workbook(item["caminho"], data_only=True, keep_links=False)
            nome_aba_origem, criterio = encontrar_aba_por_data_ou_gerencial(
                wb,
                data_selecionada,
                linha_inicial_data,
                linha_final_data,
            ...
            if not nome_aba_origem:
                logs.append(f"IGNORADO: {nome_original} não possui data real {data_selecionada.strftime('%d/%m/%Y')} em J3/K3.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue
    ...
    if not wb_final.sheetnames:
        raise Exception(f"Nenhuma aba com data real {data_selecionada.strftime('%d/%m/%Y')} em J3/K3 foi criada.")
...
def processar_multiplas_datas(
    ...
    for i, item_data in enumerate(datas_ordenadas, start=1):
        ...
        resultado = processar_arquivos_salvos(
            ...
            permitir_fallback_gerencial=permitir_fallback_gerencial,
            linha_inicial_data=linha_inicial_data,
            linha_final_data=linha_final_data,
        )
...
st.markdown(
    ...
    A data é identificada **somente nas células J3/K3** da aba. O sistema **não usa o nome da aba** para identificar datas.
    """
)
st.divider()
...
permitir_fallback_gerencial = st.sidebar.checkbox(
    "Permitir usar aba Gerencial quando J3/K3 bater com a data",
    value=False,
    help="Mesmo marcado, a aba Gerencial só será usada se as J3/K3 tiverem a mesma data selecionada."
)

st.sidebar.markdown("### Leitura da data")
st.sidebar.info("A data será lida exclusivamente em J3/K3.")
linha_inicial_data = LINHA_INICIAL_DATA_PADRAO
linha_final_data = LINHA_FINAL_DATA_PADRAO
...
if arquivos_salvos:
    ...
    if datas_para_filtro:
        ...
        st.warning("Nenhuma data foi encontrada em J3/K3 no mês/ano de referência.")
else:
...
st.caption(
    ...
    "A data é validada somente nas células J3/K3. "
