...
def obter_nome_aba_final_personalizado(nome_arquivo_origem):
    """Mapeia o arquivo para uma das 7 abas finais."""
...
def ler_data_j3_k3(aba):
    """Lê a data escrita em J3/K3. Retorna None se não houver data válida."""
    valores = []
...
def data_validada_da_aba(aba, ano_referencia, mes_referencia):
    """
    Combina as duas regras:
    1. Nome da aba indica a data: 1, 01, 0106 etc.
    2. J3/K3 confirma ou informa a data.

    Se houver data no nome e também em J3/K3, as duas precisam coincidir.
    Se aba for Gerencial, usa somente J3/K3.
    Se só houver data no nome, usa o nome.
    Se só houver data em J3/K3, usa J3/K3.
    """
...
# ============================================================
# CACHE DE DATAS: NÃO DEIXA HISTÓRICO DE OUTRO ARQUIVO DUPLICAR
# ============================================================
...
def gerar_chave_cache_datas(arquivos_salvos, ano_referencia, mes_referencia):
    ...
    return (tuple(assinatura), ano_referencia, mes_referencia, LINHA_DATA, tuple(COLUNAS_DATA), "nome_aba_mais_j3k3_com_data_principal_por_arquivo")
...
def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    """
    Para cada arquivo, determina a DATA PRINCIPAL do arquivo.
    Exemplo: CUI 06/06 pode ter histórico 04/06 e 05/06, mas a maior data validada é 06/06.
    Então este arquivo só participa do filtro/processamento de 06/06.
    """
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            status.info(f"Lendo abas e J3/K3: {idx}/{len(arquivos_salvos)} — {item['nome']}")
            ...
            # Data principal do arquivo = maior data validada encontrada nele.
            data_principal = max(r["data"] for r in validos)
            candidatos_principais = [r for r in validos if r["data"] == data_principal]

            # Preferência: aba cujo nome também indica a data principal; se não, primeira candidata.
            preferidos = [r for r in candidatos_principais if r["data_nome"] == data_principal]
            registro_usado = preferidos[0] if preferidos else candidatos_principais[0]

            for r in validos:
                if r["data"] != data_principal:
            ...
            datas_encontradas.setdefault(
                data_principal,
                {"data": datetime.combine(data_principal, datetime.min.time()), "abas": [], "arquivos": []},
            )
    ...
    status.success(f"Leitura concluída em {decorrido}. Histórico interno ignorado por arquivo.")
...
def processar_arquivos_salvos(arquivos_salvos, data_referencia, data_selecionada, copiar_estilos=True, callback_progresso=None, total_datas_selecionadas=1):
    ...
    logs.append("Regra: arquivo da data usa aba de maior data validada por arquivo e confirma nome da aba/J3-K3.")
    ...
    for item in itens_origem:
        ...
        try:
            ...
            if not nome_aba_origem:
                logs.append(f"IGNORADO: {nome_original} não é arquivo principal da data {data_selecionada.strftime('%d/%m/%Y')} ou só contém essa data como histórico.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue
            ...
            arquivos_processados.append({
                ...
                "criterio": f"Aba corresponde à data {data_selecionada.strftime('%d/%m/%Y')} e não é histórico",
            })
...
st.markdown(
    ...
    Regras principais:
    - olha a correspondência no **nome da aba**: `Gerencial`, `1`, `01`, `0106`, etc.;
    - também valida a data escrita em **J3/K3**, quando existir;
    - se uma planilha tiver histórico, o sistema usa essa planilha apenas na **maior data validada** dentro dela;
...
st.sidebar.info("Data validada por nome da aba + J3/K3. Histórico interno será ignorado.")
...
if arquivos_salvos:
    ...
    if datas_para_filtro:
        ...
        for item in datas_para_filtro:
            ...
            label = f"{data_item.strftime('%d/%m/%Y')} — data principal em {qtd_arquivos} arquivo(s), {qtd_abas} aba(s)"
...
st.caption(
    ...
    "O sistema valida a data por nome da aba e por J3/K3. "
    "Se uma planilha possui histórico, o sistema usa essa planilha somente na maior data validada dentro dela. "
    "Cada consolidado contém no máximo 7 abas: COB, CUI, EDE, NOB, PVE, SOB e XAM."
)
