...
def normalizar_nome_para_mapeamento(nome):
    """
    Normalização genérica para regras que não dependem de separadores específicos.
    """
    nome = os.path.splitext(str(nome))[0]
...
def interpretar_aba_como_data(nome_aba, ano_referencia, mes_referencia):
    ...
    try:
        if len(nome) in [1, 2]:
            dia = int(nome)
            return datetime(ano_referencia, mes_referencia, dia)
...
def listar_datas_disponiveis_arquivos(
    arquivos_salvos,
    ano_referencia,
    mes_referencia,
    quantidade_datas_filtro=5
):
    """
    Retorna somente datas mais recentes em sequência contínua
        ...
        try:
            ...
            for nome_aba in wb.sheetnames:
                ...
                if data_interpretada is not None:
                    ...
                    if chave not in datas_encontradas:
                        datas_encontradas[chave] = {
                    ...
                    datas_encontradas[chave]["arquivos"].append(nome_arquivo)
    ...
    lista_datas = list(datas_encontradas.values())

    lista_datas_mes_referencia = [
        item for item in lista_datas
        if item["data"].year == ano_referencia
        and item["data"].month == mes_referencia
    ]

    if not lista_datas_mes_referencia:
        return []

    lista_datas_mes_referencia = sorted(
        lista_datas_mes_referencia,
    ...
    for item in lista_datas_mes_referencia:
        data_atual = item["data"].date()

        if data_anterior is None:
            sequencia_continua.append(item)
            data_anterior = data_atual

            if len(sequencia_continua) >= quantidade_datas_filtro:
                break

            continue

        diferenca_dias = (data_anterior - data_atual).days
...
def encontrar_aba_por_data_ou_gerencial(
    wb_origem,
    data_selecionada,
    ano_referencia,
    mes_referencia
    ...
    """
    Regra:
    1. Primeiro tenta encontrar a aba correspondente à data selecionada.
    2. Se não encontrar, procura aba Gerencial.
    3. Se não encontrar nenhuma das duas, retorna None.
    """
    ...
    for nome_aba in wb_origem.sheetnames:
        ...
        if data_interpretada is not None and data_interpretada.date() == data_selecionada:
            ...
            abas_compativeis.append({
                "nome_aba": nome_aba,
                "prioridade": prioridade
            })

    if abas_compativeis:
        abas_compativeis = sorted(
            abas_compativeis,
            key=lambda item: item["prioridade"]
        )

        return (
            abas_compativeis[0]["nome_aba"],
            "Aba da data selecionada"
        )
    ...
    if aba_gerencial:
        return (
            aba_gerencial,
            "Aba Gerencial usada porque a aba da data selecionada não foi encontrada"
        )

    return (
        None,
        "Nenhuma aba da data selecionada ou aba Gerencial encontrada"
    )


# ============================================================
# FUNÇÕES DE UPLOAD / SESSÃO
# ============================================================
...
# ============================================================
# FUNÇÕES DE CÓPIA E FORMATAÇÃO FINAL
# ============================================================

def aplicar_configuracoes_finais_aba(aba_destino):
    """
    Aplica em todas as abas finais:
    - Zoom 80%;
    - Congelamento de painéis a partir da linha 5;
...
def padronizar_larguras_colunas_pela_aba_ede(wb_final, nome_aba_referencia="EDE"):
    """
    Padroniza a largura das colunas de todas as abas do arquivo final
    usando a aba EDE como referência.
    """

    if nome_aba_referencia not in wb_final.sheetnames:
        return False, (
            f"A aba de referência '{nome_aba_referencia}' não foi encontrada. "
            "As larguras das colunas não foram padronizadas."
        )
    ...
    return True, (
        f"Larguras das colunas padronizadas com base na aba '{nome_aba_referencia}'."
    )


def copiar_aba_como_valores_mais_leve(
    aba_origem,
    aba_destino,
    copiar_estilos=True
):
    max_linha = aba_origem.max_row
    max_coluna = aba_origem.max_column
...
def processar_arquivos_salvos(
    ...
    logs.append("Configuração final: zoom 80%, congelamento em A5, linhas de grade ocultas e larguras padronizadas pela aba EDE.")
        ...
        try:
            ...
            if not nome_aba_origem:
                ...
                raise ValueError(
                    ...
                    f"ou aba Gerencial foi encontrada. "
            ...
            nome_aba_final = gerar_nome_aba_unico(
                nome_base=nome_base_personalizado,
            ...
            logs.append(
                f"SUCESSO: aba '{nome_aba_origem}' copiada para '{nome_aba_final}'."
            )
            logs.append(f"Critério usado: {criterio_aba}")
            logs.append("Configurações aplicadas: zoom 80%, freeze panes em A5 e sem linhas de grade.")
    ...
    sucesso_larguras, mensagem_larguras = padronizar_larguras_colunas_pela_aba_ede(
        wb_final=wb_final,
        nome_aba_referencia="EDE"
    )
...
st.markdown(
    ...
    **Como funciona:**
    ...
    - Todas as abas finais ficam com **zoom 80%**, **painéis congelados em A5**, **sem linhas de grade** e **larguras de colunas iguais às da aba EDE**.
    """
)
...
with col_carregar:
    if st.button("📥 Carregar arquivos enviados", disabled=not arquivos_upload):
        try:
            ...
            st.session_state.arquivos_salvos = salvar_uploads_em_pasta_sessao(
                arquivos_upload
            )
...
if arquivos_salvos:
    ...
    if tamanho_total_mb > 30:
        st.warning(
            "Os arquivos carregados somam mais de 30 MB. "
            "Se o sistema cair, tente desmarcar a opção de copiar formatação."
        )
...
if arquivos_salvos:
    ...
    if datas_para_filtro:
        ...
        for item in datas_para_filtro:
            ...
            label = (
                f"{data_item.strftime('%d/%m/%Y')} "
                f"— encontrada em {qtd_arquivos} arquivo(s)"
            )
        ...
        opcao_escolhida = st.selectbox(
            "Selecione uma data sequencial encontrada nas abas",
        ...
        st.info(
            "Regra de cópia: o sistema tenta copiar a aba da data selecionada. "
            "Se não encontrar essa aba em algum arquivo, copia a aba Gerencial. "
            "Depois aplica zoom 80%, congela em A5, oculta linhas de grade e padroniza larguras pela aba EDE."
        )
        ...
        st.warning(
            ...
            "Se os arquivos tiverem apenas aba Gerencial, adicione pelo menos um arquivo com aba de data para aparecer no filtro."
        )
else:
...
botao_processar = st.button(
    ...
    type="primary",
    disabled=len(st.session_state.arquivos_salvos) == 0 or data_selecionada is None
)
...
if botao_processar:
    try:
        ...
        st.warning(
            "Se o app cair ou travar com vários arquivos, tente desmarcar "
            "'Copiar formatação das células' na barra lateral."
        )
...
st.caption(
    "Observação: o sistema copia a aba da data selecionada. "
    "Se algum arquivo não tiver essa aba, o sistema usa a aba Gerencial como alternativa. "
    "Todas as abas finais ficam com zoom 80%, painéis congelados em A5, linhas de grade ocultas e larguras de colunas iguais às da aba EDE. "
    "O filtro considera somente datas consecutivas no mesmo mês/ano de referência."
)
