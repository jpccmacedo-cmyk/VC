...
def interpretar_nome_aba_como_data(nome_aba, ano_referencia, mes_referencia):
    ...
    # Gerencial não tem data no nome; precisa de J3/K3.
    if nome_norm == remover_acentos(NOME_ABA_GERENCIAL).lower():
        return None
...
def data_validada_da_aba(aba, ano_referencia, mes_referencia):
    data_nome = interpretar_nome_aba_como_data(aba.title, ano_referencia, mes_referencia)
    data_j3 = ler_data_j3_k3(aba)

    # Se as duas existem, precisam coincidir. Isso evita duplicar histórico errado.
...
# ============================================================
# CACHE DE DATAS: 1 ARQUIVO = 1 ABA FINAL
# ============================================================
...
def gerar_chave_cache_datas(arquivos_salvos, ano_referencia, mes_referencia):
    ...
    return (tuple(assinatura), ano_referencia, mes_referencia, LINHA_DATA, tuple(COLUNAS_DATA), "um_arquivo_uma_aba_final")


def ler_registros_datas_arquivo(wb, ano_referencia, mes_referencia):
    registros = []
    for nome_aba in wb.sheetnames:
        aba = wb[nome_aba]
        data_validada = data_validada_da_aba(aba, ano_referencia, mes_referencia)
        data_nome = interpretar_nome_aba_como_data(nome_aba, ano_referencia, mes_referencia)
        data_j3 = ler_data_j3_k3(aba)
        registros.append({"aba": nome_aba, "data": data_validada, "data_nome": data_nome, "data_j3": data_j3})
    return registros


def escolher_registro_principal(validos):
    """
    Um arquivo deve gerar só uma aba final.
    Se tiver várias datas históricas, usa a maior data válida do arquivo.
    Dentro da maior data, prioriza aba cujo nome também indique a data.
    """
    data_principal = max(r["data"] for r in validos)
    candidatos = [r for r in validos if r["data"] == data_principal]
    preferidos = [r for r in candidatos if r["data_nome"] == data_principal]
...
def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            status.info(f"Lendo arquivo {idx}/{len(arquivos_salvos)} — {item['nome']}")
            wb = load_workbook(caminho, read_only=True, data_only=True, keep_links=False)
            registros = ler_registros_datas_arquivo(wb, ano_referencia, mes_referencia)
            validos = [r for r in registros if r["data"] is not None]
            ...
            registro_usado = escolher_registro_principal(validos)
            data_principal = registro_usado["data"]

            for r in validos:
                if r["aba"] != registro_usado["aba"]:
                    historico_ignorado.append({
                        "arquivo": item["nome"],
                        "aba": r["aba"],
                        "data": r["data"].strftime("%d/%m/%Y"),
                        "aba_usada": registro_usado["aba"],
                        "data_usada": data_principal.strftime("%d/%m/%Y"),
                    })

            if data_principal.year != ano_referencia or data_principal.month != mes_referencia:
                continue

            chave_data = data_principal.isoformat()
            mapa_datas_abas[caminho][chave_data] = registro_usado["aba"]

            datas_encontradas.setdefault(data_principal, {"data": datetime.combine(data_principal, datetime.min.time()), "abas": [], "arquivos": []})
            datas_encontradas[data_principal]["abas"].append(registro_usado["aba"])
    ...
    status.success(f"Leitura concluída em {decorrido}. Regra aplicada: 1 arquivo = 1 aba final.")
...
# ============================================================
# PROCESSAMENTO: 1 ARQUIVO = 1 ABA FINAL
# ============================================================
...
def copiar_abas_de_consolidado_existente(wb_origem, wb_final, logs, nome_arquivo, data_selecionada, ano_referencia, mes_referencia):
    ...
    for nome_aba in wb_origem.sheetnames:
        ...
        data_validada = data_validada_da_aba(aba_origem, ano_referencia, mes_referencia)
        if data_validada != data_selecionada:
            continue
...
def processar_arquivos_salvos(arquivos_salvos, data_referencia, data_selecionada, copiar_estilos=True, callback_progresso=None, total_datas_selecionadas=1):
    ...
    logs.append("Regra: 1 arquivo enviado gera no máximo 1 aba final.")
    ...
    for item in itens_consolidados:
        ...
        try:
            ...
            copiadas = copiar_abas_de_consolidado_existente(wb, wb_final, logs, item["nome"], data_selecionada, ano_ref, mes_ref)
            ...
            if copiadas:
                arquivos_processados.append({
                    ...
                    "criterio": "Abas padrão com data validada",
                })
    ...
    for item in itens_origem:
        ...
        try:
            ...
            # Este ponto é o essencial: só processa o arquivo se ele tiver data principal igual à data selecionada.
            nome_aba_origem = obter_aba_por_data_do_indice(item["caminho"], data_selecionada)
            if not nome_aba_origem:
                logs.append(f"IGNORADO: {nome_original} não é arquivo principal da data {data_selecionada.strftime('%d/%m/%Y')}.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue
            ...
            data_validada = data_validada_da_aba(aba_origem, ano_ref, mes_ref)
            if data_validada != data_selecionada:
                logs.append(f"IGNORADO: {nome_original} aba '{nome_aba_origem}' não validou para {data_selecionada.strftime('%d/%m/%Y')}.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue
            ...
            arquivos_processados.append({
                ...
                "criterio": f"1 arquivo = 1 aba final; data {data_selecionada.strftime('%d/%m/%Y')}",
            })
    ...
    if not wb_final.sheetnames:
        raise Exception(f"Nenhuma aba padrão da data {data_selecionada.strftime('%d/%m/%Y')} foi criada.")
...
def arquivos_relevantes_por_data(arquivos_salvos, data_selecionada):
    """Retorna só os arquivos cuja data principal no cache é a data selecionada."""
    mapa = st.session_state.get("cache_mapa_datas_abas", {})
    chave_data = data_selecionada.isoformat()
    return [item for item in arquivos_salvos if mapa.get(item["caminho"], {}).get(chave_data)]
...
def processar_multiplas_datas(arquivos_salvos, datas_selecionadas, data_referencia, copiar_estilos=True):
    ...
    # Total correto: se você mandou 14 arquivos, o progresso considera no máximo 14 arquivos relevantes,
    # e não 14 x quantidade de datas.
    total_etapas = sum(len(arquivos_relevantes_por_data(arquivos_salvos, item_data["data"])) for item_data in datas_ordenadas)
    ...
    def progresso(nome_arquivo, data_atual):
        ...
        tempo_box.info(f"⏱️ Tempo decorrido: {decorrido} | Tempo restante estimado: {restante} | Progresso: {etapas}/{total_etapas} arquivos")
    ...
    for i, item_data in enumerate(datas_ordenadas, start=1):
        data = item_data["data"]
        arquivos_da_data = arquivos_relevantes_por_data(arquivos_salvos, data)
        status.info(f"Gerando consolidado {i}/{len(datas_ordenadas)} para {data.strftime('%d/%m/%Y')} com {len(arquivos_da_data)} arquivo(s)")
...
st.markdown(
    ...
    - **1 arquivo enviado gera no máximo 1 aba final**.
    - Se você enviar **14 planilhas**, o sistema vai gerar **14 abas no total** entre os consolidados, não 28.
    - Se forem 7 planilhas do dia 05/06 e 7 planilhas do dia 06/06, serão 2 consolidados com 7 abas cada.
    - O histórico interno das planilhas é usado só para descobrir a aba principal do arquivo e depois é ignorado.
    """
)
st.divider()
...
st.sidebar.info("Data validada por nome da aba + J3/K3. Regra: 1 arquivo = 1 aba final.")
...
if arquivos_salvos:
    ...
    if datas_para_filtro:
        ...
        for item in datas_para_filtro:
            ...
            label = f"{data_item.strftime('%d/%m/%Y')} — {qtd_arquivos} arquivo(s) / {qtd_abas} aba(s) finais"
        ...
        if datas_selecionadas:
            total_arquivos_relevantes = sum(len(arquivos_relevantes_por_data(arquivos_salvos, item["data"])) for item in datas_selecionadas)
            st.success(f"{len(datas_selecionadas)} data(s) selecionada(s).")
            st.info(f"Serão processados {total_arquivos_relevantes} arquivo(s), gerando no máximo {total_arquivos_relevantes} aba(s) no total.")
    else:
        st.warning("Nenhuma data principal foi encontrada no mês/ano de referência.")

    if historico_ignorado:
        with st.expander("Histórico ignorado dentro das planilhas"):
            for item in historico_ignorado[:200]:
                st.write(f"- **Arquivo:** {item['arquivo']} | **Aba ignorada:** {item['aba']} | **Data ignorada:** {item['data']} | **Aba usada:** {item['aba_usada']} | **Data usada:** {item['data_usada']}")
            if len(historico_ignorado) > 200:
                st.write(f"... e mais {len(historico_ignorado) - 200} registro(s).")

    if abas_sem_data:
        with st.expander("Abas sem data identificável pelo nome ou J3/K3"):
...
st.divider()
st.caption(
    "Regra final: 1 arquivo enviado gera no máximo 1 aba final. "
    "Se você enviar 14 planilhas, o processamento considera 14 arquivos relevantes e não duplica abas por histórico. "
