...
def obter_nome_aba_final_personalizado(nome_arquivo_origem):
    """Mapeia o arquivo para uma das 7 abas padrão."""
    ...
    # Se não casar com regra, tenta extrair se o nome já começa com uma aba padrão.
...
def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    """
    Lê J3/K3 uma vez e guarda:
    - datas reais disponíveis;
    - mapa caminho -> data -> aba;
    - abas sem data válida.
    """
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            status.info(f"Lendo datas reais em J3/K3: {idx}/{len(arquivos_salvos)} — {item['nome']}")
            ...
            for nome_aba in wb.sheetnames:
                ...
                if data_real is None:
                    abas_sem_data.append({"arquivo": item["nome"], "aba": nome_aba})
                    continue
                ...
                # Guarda a primeira aba daquela data nesse arquivo. Se houver outra, fica no log via abas.
    ...
    return lista_datas[:quantidade_datas_filtro], mapa_datas_abas, abas_sem_data
...
def limpar_cache_datas():
    ...
    st.session_state.pop("cache_abas_sem_data", None)
...
if arquivos_salvos:
    datas_para_filtro, mapa_datas_abas, abas_sem_data = construir_indice_datas(
        arquivos_salvos,
