...
# REGRA FINAL DE DATA: somente J3/K3. Não usa nome da aba.
LINHA_DATA = 3
...
def obter_nome_aba_final_personalizado(nome_arquivo_origem):
    """Mapeia nome de arquivo para nome final da aba."""
...
def ler_data_j3_k3(aba):
    """
    Lê exclusivamente J3/K3.
    Não usa nome da aba, não lê outras linhas/colunas.
    """
    valores = []
...
# ============================================================
# ÍNDICE DE DATAS OTIMIZADO COM CACHE
# ============================================================
...
def gerar_chave_cache_datas(arquivos_salvos, ano_referencia, mes_referencia):
    ...
    return (tuple(assinatura), ano_referencia, mes_referencia, LINHA_DATA, tuple(COLUNAS_DATA))


def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    """
    Otimização principal:
    - Abre cada arquivo uma vez em modo read_only.
    - Lê somente J3/K3 de cada aba.
    - Salva mapa em st.session_state para não reler a cada interação do Streamlit.
    """
    ...
    if (
        ...
        return st.session_state.cache_datas_opcoes[:quantidade_datas_filtro], st.session_state.cache_mapa_datas_abas
        ...
        try:
            ...
            for nome_aba in wb.sheetnames:
                ...
                if data_real and data_real.year == ano_referencia and data_real.month == mes_referencia:
                    ...
                    mapa_datas_abas[caminho].setdefault(chave_data, nome_aba)
    ...
    status.success(f"Leitura de datas concluída em {decorrido}. Resultado em cache para esta carga de arquivos.")
    ...
    return lista_datas[:quantidade_datas_filtro], mapa_datas_abas
...
def extrair_data_nome_consolidado(nome_arquivo):
    ...
    A escolha de abas continua sendo somente por J3/K3.
    """
...
def salvar_uploads_em_pasta_sessao(arquivos_upload):
    ...
    # Limpa cache ao carregar nova remessa.
    st.session_state.pop("cache_datas_chave", None)
    st.session_state.pop("cache_datas_opcoes", None)
    st.session_state.pop("cache_mapa_datas_abas", None)
...
def processar_arquivos_salvos(arquivos_salvos, data_referencia, data_selecionada, copiar_estilos=True, callback_progresso=None, total_datas_selecionadas=1, permitir_fallback_gerencial=False):
    ...
    logs.append("Validação única: data lida exclusivamente em J3/K3. Nome da aba não é considerado.")
    ...
    for item in itens_origem:
        ...
        try:
            ...
            # Usa o índice em cache para não varrer as abas novamente.
            nome_aba_origem = obter_aba_por_data_do_indice(item["caminho"], data_selecionada)

            # Se o cache ainda não tiver a aba, faz uma checagem rápida apenas em J3/K3.
            ...
            arquivos_processados.append({
                ...
                "criterio": "Aba selecionada por data validada em J3/K3",
            })
...
st.markdown(
    ...
    A data é identificada **somente em J3/K3** da aba. O sistema **não usa o nome da aba** para identificar datas.

    A leitura das datas foi otimizada com **cache**: após carregar os arquivos, o app lê J3/K3 uma vez e reutiliza o resultado nas próximas interações.
    """
)
st.divider()
...
permitir_fallback_gerencial = st.sidebar.checkbox(
    ...
    value=False,
    help="Mesmo marcado, a aba Gerencial só será usada se J3/K3 tiver a mesma data selecionada."
)
st.sidebar.info("Leitura da data fixa em J3/K3.")
...
if arquivos_salvos:
    datas_para_filtro, mapa_datas_abas = construir_indice_datas(
        arquivos_salvos,
...
st.caption(
    ...
    "A data é validada somente em J3/K3. "
    "O sistema não usa o nome da aba para identificar data. "
    "A leitura inicial das datas usa cache para evitar reler os arquivos em cada interação. "
