...
def interpretar_nome_aba_como_data(nome_aba, ano_referencia, mes_referencia):
    ...
    # Gerencial não tem data no nome; só entra por J3/K3.
...
def data_validada_da_aba(aba, ano_referencia, mes_referencia):
    """
    Regra combinada:
    - Nome da aba indica a data: 1, 01, 0106 etc.
    - J3/K3 valida quando existir.
    - Se houver data no nome e em J3/K3, as duas precisam coincidir.
    - Gerencial só entra se J3/K3 tiver a data.
    """
...
# ============================================================
# CACHE DE DATAS: DATA PRINCIPAL POR ARQUIVO
# ============================================================
...
def gerar_chave_cache_datas(arquivos_salvos, ano_referencia, mes_referencia):
    ...
    return (tuple(assinatura), ano_referencia, mes_referencia, LINHA_DATA, tuple(COLUNAS_DATA), "principal_por_arquivo_nome_aba_j3k3")
...
def construir_indice_datas(arquivos_salvos, ano_referencia, mes_referencia, quantidade_datas_filtro):
    """
    Para cada arquivo, define a DATA PRINCIPAL do arquivo.
    Exemplo: arquivo CUI 06/06 com histórico 04/06, 05/06 e 06/06 -> data principal 06/06.
    Assim esse arquivo só é usado no consolidado 06/06.
    """
    ...
    for idx, item in enumerate(arquivos_salvos, start=1):
        ...
        try:
            ...
            # DATA PRINCIPAL = maior data validada dentro desse arquivo.
            data_principal = max(r["data"] for r in validos)
            candidatos_principais = [r for r in validos if r["data"] == data_principal]

            # Preferência: aba cujo nome também indica a data principal.
...
with col_carregar:
    if st.button("F4E5 Carregar arquivos enviados".replace("\u0001F4E5", "📥"), disabled=not arquivos_upload):
        try:
