...
# ============================================================
# LEITURA DE DATA DENTRO DA PLANILHA
# ============================================================
...
def ler_texto_intervalo(aba, celulas):
    partes = []
    for ref in celulas:
        try:
            valor = aba[ref].value
...
def extrair_data_da_aba_pelo_conteudo(aba):
    """
    Lê a data dentro da própria planilha.

    AJUSTE PRINCIPAL:
    - Primeiro lê exatamente J5 e K5, conforme informado pelo usuário.
    - Também lê combinações ao redor de J5/K5 para casos de célula mesclada ou deslocada.
    - Depois faz varredura curta nas primeiras linhas.
    """
    # 1) Prioridade absoluta: J5/K5.
    grupos_prioritarios = [
        ["J5", "K5"],
        ["I5", "J5", "K5", "L5"],
        ["J4", "K4", "J5", "K5", "J6", "K6"],
    ...
    for grupo in grupos_prioritarios:
        texto = ler_texto_intervalo(aba, grupo)
        data_texto = interpretar_data_em_texto(texto)
        if data_texto:
            return data_texto
        for ref in grupo:
            try:
                data_valor = converter_para_date(aba[ref].value)
                if data_valor:
                    return data_valor
            except Exception:
                pass

    # 2) Procura o rótulo Data nas primeiras linhas, principalmente J/K.
    max_linha = min(aba.max_row or 1, 12)
    max_coluna = min(aba.max_column or 1, 15)  # até O
            ...
            if valor_texto.startswith("data") or valor_texto == "data:":
                ...
                for col_busca in range(coluna, min(max_coluna, coluna + 8) + 1):
                    ...
                    if valor_busca is not None:
                        textos.append(str(valor_busca))
                data_texto = interpretar_data_em_texto(" ".join(textos))
                if data_texto:
                    return data_texto

    # 3) Varredura geral no topo da planilha.
    for linha in range(1, max_linha + 1):
        textos_linha = []
        for coluna in range(1, max_coluna + 1):
            valor = aba.cell(row=linha, column=coluna).value
            ...
            if data_valor:
                return data_valor
            if valor is not None:
                textos_linha.append(str(valor))
        data_texto = interpretar_data_em_texto(" ".join(textos_linha))
        if data_texto:
            return data_texto
...
def obter_data_real_da_aba(aba, ano_referencia, mes_referencia):
    ...
    return data_nome.date() if data_nome else None
...
def encontrar_aba_exata_por_data(wb_origem, data_selecionada, ano_referencia, mes_referencia):
    ...
    if candidatas:
        ...
        return candidatas[0]["nome_aba"], "Aba com data real lida em J5/K5 igual à data selecionada"
...
def encontrar_aba_por_data_ou_gerencial(wb_origem, data_selecionada, ano_referencia, mes_referencia, permitir_fallback_gerencial=False):
    nome_aba, criterio = encontrar_aba_exata_por_data(wb_origem, data_selecionada, ano_referencia, mes_referencia)
    if nome_aba:
        return nome_aba, criterio
    if permitir_fallback_gerencial:
        aba_gerencial = encontrar_aba_gerencial(wb_origem)
        if aba_gerencial:
            return aba_gerencial, "Aba Gerencial usada por opção do usuário"
    return None, "Nenhuma aba com data real igual à data selecionada encontrada"
...
def copiar_abas_de_consolidado_existente(wb_origem, wb_final, logs, nome_arquivo, data_selecionada, ano_referencia, mes_referencia):
    ...
    for nome_aba in wb_origem.sheetnames:
        ...
        if data_real is not None and data_real != data_selecionada:
            logs.append(f"Consolidado existente: aba '{nome_aba}' ignorada porque a data real é {data_real.strftime('%d/%m/%Y')}.")
            continue
...
def processar_arquivos_salvos(arquivos_salvos, data_referencia, data_selecionada, copiar_estilos=True, callback_progresso=None, total_datas_selecionadas=1, permitir_fallback_gerencial=False):
    ...
    ano_ref, mes_ref = data_referencia.year, data_referencia.month
    ...
    logs.append("Validação principal: data lida em J5/K5 e arredores.")

    itens_consolidados, itens_origem = [], []
    for item in arquivos_salvos:
        (itens_consolidados if detectar_consolidado_existente(item) else itens_origem).append(item)

    # Consolidados existentes entram somente se forem da mesma data.
        ...
        try:
            ...
            if copiadas:
                arquivos_processados.append({
                    "arquivo": item["nome"], "aba_origem": "Consolidado existente",
                    "aba_final": ", ".join(copiadas),
                    "criterio": "Abas reaproveitadas de consolidado existente da mesma data"
                })
    ...
    for item in itens_origem:
        ...
        try:
            ...
            wb = load_workbook(item["caminho"], data_only=True, keep_links=False)
            nome_aba_origem, criterio = encontrar_aba_por_data_ou_gerencial(wb, data_selecionada, ano_ref, mes_ref, permitir_fallback_gerencial)

            if not nome_aba_origem:
                logs.append(f"IGNORADO: {nome_original} não possui data real {data_selecionada.strftime('%d/%m/%Y')} em J5/K5 ou arredores.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue

            aba_origem = wb[nome_aba_origem]
            data_real = obter_data_real_da_aba(aba_origem, ano_ref, mes_ref)
            if data_real and data_real != data_selecionada:
                logs.append(f"IGNORADO: {nome_original} tem data real {data_real.strftime('%d/%m/%Y')}, não {data_selecionada.strftime('%d/%m/%Y')}.")
                if callback_progresso:
                    callback_progresso(nome_original, data_selecionada)
                continue
    ...
    if not wb_final.sheetnames:
        raise Exception(f"Nenhuma aba com data real {data_selecionada.strftime('%d/%m/%Y')} foi criada.")
...
def processar_multiplas_datas(arquivos_salvos, datas_selecionadas, data_referencia, copiar_estilos=True, permitir_fallback_gerencial=False):
    ...
    for i, item_data in enumerate(datas_ordenadas, start=1):
        ...
        resultado = processar_arquivos_salvos(
            arquivos_salvos, data_referencia, data,
...
st.markdown(
    ...
    A validação principal da data agora lê a região **J5/K5** da aba, como no cabeçalho:
...
permitir_fallback_gerencial = st.sidebar.checkbox(
    ...
    value=False,
    help="Deixe desmarcado para garantir que cada consolidado use somente abas com data real em J5/K5 igual à data selecionada."
)
...
if arquivos_salvos:
    datas_para_filtro = listar_datas_disponiveis_arquivos(
        arquivos_salvos, data_referencia.year, data_referencia.month, quantidade_datas_filtro
    )
    if datas_para_filtro:
        ...
        st.warning("Nenhuma data foi encontrada em J5/K5, arredores ou nomes de abas no mês/ano de referência.")
else:
...
st.caption(
    ...
    "A data é validada prioritariamente nas células J5/K5 e arredores. "
